...
function buildViewsWindow(videos: NonNullable<CustomerPulseInitialData['tiktok_stats']>['recent_videos'] = [], days = 30) {
  const end = new Date();
  ...
  for (const v of videos) {
    const key = new Date(v.uploaded_at).toISOString().slice(0, 10);
    if (buckets.has(key)) buckets.set(key, (buckets.get(key) ?? 0) + (v.views ?? 0));
    totalViews += v.views ?? 0;
    totalLikes += v.likes ?? 0;
  }
  return {
    labels,
    values: labels.map((l) => buckets.get(l) ?? 0),
    totalViews,
...
export function CustomerDriftRoute({ customerId, overview, pulse }: DriftRouteProps) {
  const [changeCmOpen, setChangeCmOpen] = useState(false);

  const tStats = pulse.tiktok_stats;
  ...
  return (
    <Stack gap="lg">
        ...
        <Card withBorder padding="md">
          ...
          <SimpleGrid cols={{ base: 1, md: 2 }} spacing="lg">
            <Paper withBorder p="md" bg="gray.0">
              ...
              <PerformanceChart
                data={tStats.history.map((p) => p.followers)}
                labels={tStats.history.map((p) => p.snapshot_date)}
      ...
      {/* CM-block + Senaste publiceringar */}
      <Grid grow>
                  ...
                  <Text size="sm">
                    {pulse.last_cm_action_at ? (
                      <>
                        ...
                        {pulse.last_cm_action_type && <> · {pulse.last_cm_action_type}</>}
                      </>
                    ) : (
        ...
        <Grid.Col span={{ base: 12, md: 6 }}>
          <Card withBorder padding="md" h="100%">
            ...
            {pulse.recent_publications.length === 0 ? (
              ...
              <Stack gap="xs">
                {pulse.recent_publications.slice(0, 6).map((pub) => (
                  <Group
                    key={pub.id}
                  ...
                  >
                    <Box style={{ minWidth: 0, flex: 1 }}>
                      <Text size="sm" fw={500} lineClamp={1}>
                        {pub.title || 'Utan titel'}
                      </Text>
                      <Text size="xs" c="dimmed">
                        {format(new Date(pub.published_at), "d MMM yyyy 'kl' HH:mm", {
                          locale: sv,
                        })}
                      </Text>
                    </Box>
      ...
      <ChangeCMModal
        ...
        currentAccountManagerId={overview.account_manager_member_id}
