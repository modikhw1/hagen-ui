...
export default function SubscriptionModal({
  ...
  // Reset state vid open
  useEffect(() => {
    if (open) {
      setPriceOre(currentPriceOre);
      setPriceMode('next_period');
      setAction('none');
      setPauseUntil(customer.paused_until ?? '');
    }
  }, [open, currentPriceOre, customer.paused_until]);
  ...
  const skippedInvoice = useMemo(() => {
    ...
    const skipDate = customer.next_invoice_date;
    const skipAmount = currentPriceOre;
    return { date: skipDate, amount_ore: skipAmount };
  ...
  const isPaused = !!customer.paused_until;
  const isCanceledAtEnd = !!subscription?.cancel_at_period_end;
      ...
      >
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3 rounded-md border border-border bg-muted/30 p-3 text-xs">
            ...
            <KeyVal
              label="Nästa fakturering"
              value={fmtDate(customer.next_invoice_date)}
            />
            <KeyVal
              label="Status"
              value={
              ...
              {preview && !previewLoading && (
                <div className="rounded border border-border bg-muted/40 p-3 text-xs">
                  ...
                  {preview.mode === 'next_period' ? (
                    <p>
                      Nytt pris{' '}
                      <strong>{fmtKr(preview.new_price_ore)}/mån</strong> börjar gälla{' '}
                      <strong>{fmtDate(preview.effective_date)}</strong>. Ingen
                      direkt fakturering.
                    </p>
                  ) : (
          ...
          {/* Sektion: Cancel (båda lägen) */}
          {cancelMode && (
            <div className="space-y-2 rounded-md border border-border p-3">
              <CancelPreviewPanel customerId={customerId} mode={cancelMode} />
              {action === 'cancel_now' && (
                <Alert color="red" icon={<Info className="h-4 w-4" />}>
                  Omedelbart avslut stänger tillgång direkt. Vid prorata-belopp
                  ovan föreslås en kreditnota.
                </Alert>
              )}
            </div>
          )}
            ...
            <div className="space-y-3 rounded-md border border-border p-3">
              ...
              {skippedInvoice && (
                <Alert color="blue" icon={<CalendarClock className="h-4 w-4" />}>
                  ...
                  <strong>{fmtDate(skippedInvoice.date)}</strong> på{' '}
