...
import { Modal, Button, TextInput, Stack, Group, Text, Alert, Radio, Box, Divider, Skeleton } from '@mantine/core';
import { IconCalendarStats, IconAlertTriangle, IconReceipt } from '@tabler/icons-react';
...
export function UpdatePricingDialog({
  ...
  useEffect(() => {
    if (open) {
      setNewPriceKr(String(Math.round(currentPriceOre / 100)));
      setEffectiveDate('next_cycle');
    }
  }, [open, currentPriceOre]);
      ...
      <Stack gap="md">
        ...
        >
          <Stack gap="xs" mt="xs">
            ...
            <Radio value="immediate" label={<Text size="sm">Omedelbart (proportionerlig fakturering)</Text>} />
          </Stack>
        </Radio.Group>
        ...
        {previewError && (
          <Alert color="red" icon={<IconAlertTriangle size={16} />} title="Kunde inte hämta förhandsgranskning">
