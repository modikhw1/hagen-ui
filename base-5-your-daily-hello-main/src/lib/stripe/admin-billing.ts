...
export interface DiscountLineItemInput {
  type: 'amount' | 'percent' | 'free_months';
  /** SEK per månad (amount) eller procent (percent). Ignoreras för free_months. */
  value?: number;
  /** Antal kommande fakturor rabatten ska gälla på. */
  months: number;
  /** Valfri egen beskrivning som syns på fakturan. */
  description?: string;
  idempotencyToken?: string | null;
}
...
/**
 * Lägger en rabatt som negativa invoice items på kommande fakturor.
 * Föredragen modell framför coupon för engångs- och tidsbegränsade rabatter:
 * varje rad är synlig på fakturan, lätt att ångra (delete pending invoice item)
 * och kräver ingen separat coupon/promotionCode-livscykel.
 *
 * Skapar `months` st pending invoice items, en per kommande faktureringsperiod.
 * Stripe drar dem på nästa subscription-faktura i FIFO-ordning (1 per period).
 */
export async function applyDiscountAsLineItem(
  args: Ctx & {
    profileId: string;
    input: DiscountLineItemInput;
  },
) {
  const stripe = ensureStripe(args);
  const profile = await getProfileWithStripe(args, args.profileId);
  ...
  const { type, months, description, idempotencyToken } = args.input;
  const safeMonths = Math.max(1, Math.min(12, Math.round(months)));

  let perMonthOre: number;
  if (type === 'amount') {
    if (!args.input.value || args.input.value <= 0) {
      throw new Error('Belopp måste anges för rabatt-typ "amount"');
    }
    perMonthOre = -Math.round(args.input.value * 100);
  } else if (type === 'percent') {
    if (!args.input.value || args.input.value <= 0 || args.input.value > 100) {
      throw new Error('Procent måste vara 1–100');
    }
    if (!profile.stripe_subscription_id) {
      throw new Error(
        'Procentrabatt kräver aktivt abonnemang för att räkna ut månadsbelopp',
      );
    }
    const { item } = await getSubscriptionContext(args);
    const recurringInterval = item.price.recurring?.interval ?? 'month';
    const recurringIntervalCount = item.price.recurring?.interval_count ?? 1;
    const monthlyOre = monthlyAmountOreFromRecurringUnit({
      unitAmountOre: item.price.unit_amount ?? 0,
      interval: recurringInterval,
      intervalCount: recurringIntervalCount,
    });
    perMonthOre = -Math.round((monthlyOre * args.input.value) / 100);
  } else {
    if (!profile.stripe_subscription_id) {
      throw new Error('Gratis månader kräver aktivt abonnemang');
    }
    const { item } = await getSubscriptionContext(args);
    const recurringInterval = item.price.recurring?.interval ?? 'month';
    const recurringIntervalCount = item.price.recurring?.interval_count ?? 1;
    perMonthOre = -monthlyAmountOreFromRecurringUnit({
      unitAmountOre: item.price.unit_amount ?? 0,
      interval: recurringInterval,
      intervalCount: recurringIntervalCount,
    });
  ...
  if (perMonthOre === 0) {
    throw new Error('Beräknat rabattbelopp är 0 – avbryter');
  }
  ...
  const baseDescription =
    ...
    (type === 'free_months'
      ? 'Gratis månad'
      : type === 'percent'
        ? `Rabatt ${args.input.value}%`
        : `Rabatt ${args.input.value} kr`);

  const baseIdem =
    idempotencyToken ??
    `discount-li:${args.profileId}:${type}:${args.input.value ?? 0}:${safeMonths}`;

  const createdItems: Array<{ id: string; amount_ore: number }> = [];

  for (let i = 0; i < safeMonths; i += 1) {
    const lineDescription =
      safeMonths > 1 ? `${baseDescription} (${i + 1}/${safeMonths})` : baseDescription;
    ...
    const item = await stripe.invoiceItems.create(
      {
        ...
        metadata: {
          customer_profile_id: args.profileId,
          discount_strategy: 'line_item',
          discount_type: type,
          discount_value: String(args.input.value ?? ''),
          discount_index: String(i + 1),
          discount_total_months: String(safeMonths),
        },
      },
      { idempotencyKey: `${baseIdem}:${i + 1}` },
    );

    createdItems.push({ id: item.id, amount_ore: item.amount });
  }

  await logStripeAdminAction(args.supabaseAdmin, {
    eventType: 'admin.discount.applied_line_item',
    objectType: 'customer',
    objectId: profile.stripe_customer_id,
    status: 'success',
    summary: {
      ...
      months: safeMonths,
      per_month_ore: perMonthOre,
      total_ore: perMonthOre * safeMonths,
      invoice_item_ids: createdItems.map((it) => it.id),
      idempotency_token: idempotencyToken ?? null,
    },
  });
  ...
  return {
    ...
    perMonthOre,
    months: safeMonths,
    totalOre: perMonthOre * safeMonths,
  };
}
