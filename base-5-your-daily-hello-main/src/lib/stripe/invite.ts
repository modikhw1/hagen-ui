...
async function findExistingUserByEmail(
  ...
  // För 0–100 kunder räcker en enda sida (perPage=200). Om vi någonsin närmar oss
  // den gränsen byter vi till en direkt query mot auth.users via service role.
...
export async function sendCustomerInvite(params: {
  ...
  if (existingAuthUser?.email_confirmed_at) {
    return {
      ok: false,
      status: 409,
      error: `${payload.contact_email} har redan ett verifierat konto. Länka profilen manuellt eller använd en annan e-post.`,
