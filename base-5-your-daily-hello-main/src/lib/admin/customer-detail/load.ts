...
export async function loadAdminCustomerHeader(id: string) {
  return unstable_cache(
    async () => {
      ...
      const [profileResult, bufferResult] = await Promise.all([
        supabaseAdmin
          ...
          .select(`
            ...
            escalation_flag, concepts_per_week, expected_concepts_per_week,
            pricing_status,
      ...
      const derivedStatus = deriveCustomerStatus({
        ...
        escalation_flag: data.escalation_flag ?? null,
      });
      ...
      return {
        ...
        account_manager_name: (data.account_manager as string | null) ?? null,
        ...
        escalation_flag: Boolean(data.escalation_flag),
