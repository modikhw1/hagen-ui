...
/**
 * Leverans 5: Lista senaste reconcile-jobb (köade + slutförda).
 * Används av Health-vyns "Reconcile-historik".
 */
...
export const GET = withAuth(async (request: NextRequest, user) => {
  ...
  const baseQuery = table
    .select('id, scope, environment, status, since, payload, result, error_message, created_at, started_at, finished_at, requested_by')
  ...
  if (result.error) {
    if (isMissingRelationError(result.error.message)) {
      return new Response(
        JSON.stringify({ jobs: [], schemaWarning: 'Tabellen admin_billing_reconcile_jobs saknas. Kör migration.' }),
