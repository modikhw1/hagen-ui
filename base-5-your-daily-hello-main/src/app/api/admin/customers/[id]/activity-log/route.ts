...
function buildAuditDescription(entry: {
  ...
  if (
    entry.action === 'admin.customer.cm_changed' ||
    entry.action === 'admin.customer.cm_change_scheduled'
  ) {
    const date =
  ...
  if (entry.action === 'admin.customer.subscription_price_changed') {
    const monthlyPrice =
      typeof entry.metadata?.monthly_price === 'number'
  ...
  if (entry.action === 'admin.customer.discount_applied') {
    const type =
      typeof entry.metadata?.type === 'string' ? entry.metadata.type : 'discount';
    const value =
  ...
  if (entry.action === 'system.customer.discount_expired') {
    const endDate =
      typeof entry.metadata?.discount_end_date === 'string'
        ? entry.metadata.discount_end_date
        : null;
    return endDate
  ...
  if (entry.action === 'admin.invoice.created') {
    const itemCount =
      typeof entry.metadata?.item_count === 'number'
    ...
    try {
      ...
      const [
        ...
        supabaseAdmin
          ...
          .limit(12),
        // admin_customer_notes — kan saknas tills migrationen är körd
        (supabaseAdmin.from as any)('admin_customer_notes')
          ...
          .limit(50)
          .then(
            (r: { data: any[] | null; error: { message?: string } | null }) =>
              r.error ? { data: [], error: null, missing: true } : { ...r, missing: false },
