  ...
  try {
    ...
    switch (event.type) {
      ...
      case 'invoice.updated': {
        ...
        // Differentierad signal: finalized = "officiell faktura skapad / skickad
        // till kund". Värdefullt att kunna filtrera ut separat i sync-log.
        if (event.type === 'invoice.finalized') {
          appliedChanges.finalized = true;
          appliedChanges.invoice_number = invoice.number ?? null;
          appliedChanges.hosted_invoice_url = invoice.hosted_invoice_url ?? null;
        }
      ...
      case 'checkout.session.completed': {
        // Första betalningen / onboarding-checkout landade. Vi muterar inte
        // state här — subscription/invoice-events täcker det. Men vi vill att
        // admin-orkestratorn ser ett strukturerat "köp landade"-event istället
        // för att det faller i default som 'skipped'.
        const session = event.data.object as Stripe.Checkout.Session;
        resolvedCustomerProfileId = await resolveCustomerProfileId({
          supabaseAdmin,
          stripeCustomerId: readStripeCustomerId(session.customer ?? null),
          stripeSubscriptionId: readStripeSubscriptionId(
            (session as Stripe.Checkout.Session & { subscription?: string | Stripe.Subscription | null })
              .subscription ?? null,
          ),
        });
        appliedChanges.checkout_session_id = session.id;
        appliedChanges.checkout_mode = session.mode;
        appliedChanges.checkout_payment_status = session.payment_status;
        appliedChanges.amount_total = session.amount_total ?? null;
        appliedChanges.client_reference_id = session.client_reference_id ?? null;
        break;
      }

      case 'payment_intent.succeeded': {
        // För subscription-fakturor är detta redundant gentemot invoice.paid.
        // Men för manuella charges utanför invoice är det enda signalen.
        // Vi loggar alltid så orkestratorn har en sanning att luta sig mot,
        // men muterar ingen state — invoice/charge-spegling sker via sina
        // egna events.
        const intent = event.data.object as Stripe.PaymentIntent;
        resolvedCustomerProfileId = await resolveCustomerProfileId({
          supabaseAdmin,
          stripeCustomerId: readStripeCustomerId(intent.customer),
        });
        appliedChanges.payment_intent_id = intent.id;
        appliedChanges.amount_received = intent.amount_received;
        appliedChanges.has_invoice = Boolean(
          (intent as Stripe.PaymentIntent & { invoice?: string | null }).invoice,
        );
        break;
      }
