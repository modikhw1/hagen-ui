...
export default function ReconcileJobsList() {
  ...
  const { data, isLoading } = useQuery({
    ...
    refetchInterval: (query) => {
      const jobs = (query.state.data as ListResponse | undefined)?.jobs ?? [];
  ...
  const runMutation = useMutation({
    mutationFn: (jobId?: string) =>
      apiClient.post<{ ok: boolean; status?: string; error?: string }>(
        '/api/admin/billing/reconcile/run',
        jobId ? { jobId } : {},
      ),
    onSuccess: (result) => {
      if (result.ok) {
        toast.success(`Reconcile klar (${result.status ?? 'ok'})`);
      } else {
        toast.error(result.error ?? 'Reconcile misslyckades');
      }
            ...
            <li key={job.id} className="flex items-start justify-between gap-3 px-4 py-2.5 text-xs">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span
                    className={`rounded px-1.5 py-0.5 text-[10px] font-medium uppercase ${statusStyles[job.status]}`}
                  >
                    {job.status}
                  </span>
                ...
                {job.result ? (
                  <div className="mt-1 text-muted-foreground">
                    ...
                    {typeof job.result.durationMs === 'number'
                      ? ` · ${Math.round(job.result.durationMs / 100) / 10}s`
                      : ''}
                  </div>
                ) : null}
                {job.error_message ? (
                ...
                <div className="mt-1 flex items-center gap-3">
                  <a
                    href="/admin/audit-log?entity=billing"
