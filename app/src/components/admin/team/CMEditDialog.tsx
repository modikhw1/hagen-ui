    ...
    await Promise.all(
      cm.customers.map(async (customer) => {
        ...
        if (!result.ok) {
          ...
          throw new Error(payload.error || 'Misslyckades att omfördela kunder');
        }
      })
    );
  };
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/team/${cm.id}`, {
        ...
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({ name, email, phone, city, bio, avatar_url: avatarUrl }),
      });
      if (!response.ok) {
    ...
    <Dialog open={open} onOpenChange={(nextOpen) => !nextOpen && onClose()}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Uppdatera profil och hantera kunder
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="mb-4 flex items-center gap-4">
          ...
          <div className="flex-1">
            ...
            <div className="mt-1 text-[11px] text-muted-foreground">Klicka på bilden för att byta profilbild</div>
          </div>
        </div>
