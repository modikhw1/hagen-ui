  ...
  return (
    <Paper shadow="sm" radius="lg" withBorder style={{ height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <Group p="md" style={{ borderBottom: "1px solid #eee" }}>
        <Avatar style={{ backgroundColor: "#4A2F18" }} radius="xl">L</Avatar>
        <div>
        ...
        <Stack gap="md">
          {messages.map((msg) => (
            <div key={msg.id}>
              <Group align="flex-start" gap="sm" style={{ flexDirection: msg.role === "user" ? "row-reverse" : "row" }}>
                {msg.role === "assistant" && (
                  <Avatar style={{ backgroundColor: "#4A2F18" }} radius="xl" size="sm">L</Avatar>
                )}
                <Paper
                  p="sm"
                  radius="md"
                  style={{ backgroundColor: msg.role === "user" ? "#4A2F18" : "#F5F2EE" }}
                  c={msg.role === "user" ? "white" : undefined}
                  maw="80%"
                >
                  <Text size="sm" style={{ whiteSpace: "pre-line" }}>{msg.content}</Text>
                </Paper>
              </Group>
                ...
                <Group gap="xs" mt="sm" ml={40}>
                  {msg.options.map((option) => (
                    <Badge
                      ...
                      variant="outline"
                      size="lg"
                      style={{ cursor: "pointer", borderColor: "#4A2F18", color: "#4A2F18" }}
          ...
          {isTyping && (
            <Group align="flex-start" gap="sm">
              <Avatar style={{ backgroundColor: "#4A2F18" }} radius="xl" size="sm">L</Avatar>
              <Paper p="sm" radius="md" style={{ backgroundColor: "#F5F2EE" }}>
                ...
                <Stack gap="sm">
                  ...
                  <Group gap="xs">
                    <Button
                      ...
                      style={{ backgroundColor: socialInput.trim() ? "#4A2F18" : undefined }}
                    >
                      Connect TikTok
                    </Button>
                    <Button
                      ...
                      disabled={!socialInput.trim()}
                      style={{ backgroundColor: "#F0EBE4", color: "#4A2F18" }}
                    >
                      Connect Instagram
                    </Button>
                    <Button size="xs" variant="subtle" onClick={handleSkipSocial} c="#7D6E5D">
                      Skip
                    </Button>
                  </Group>
                </Stack>
      ...
      <Group p="md" style={{ borderTop: "1px solid #eee" }}>
        ...
        <Button
          onClick={handleSend}
          disabled={!input.trim() || showSocialSync || stage === "complete"}
          style={{ backgroundColor: input.trim() && !showSocialSync && stage !== "complete" ? "#4A2F18" : undefined }}
        >
          <IconSend size={18} />
        </Button>
      </Group>
