...
export async function GET(request: NextRequest) {
  try {
    ...
    if (customers.data.length === 0) {
      // No customer found - no pending agreement
    ...
    if (activeSubscription) {
      ...
      // Check if active subscription has an unpaid invoice (invoice-based billing)
      ...
      if (subscriptionInvoices.data.length > 0) {
        // Active subscription but first invoice not paid yet
        ...
        return NextResponse.json({
          agreement: {
            ...
            scopeItems: safeJsonParse<string[] | null>(productInfo.metadata.scope_items, null),
            hostedInvoiceUrl: invoice.hosted_invoice_url,
          },
        });
      }

      // Fully active subscription - all invoices paid
      return NextResponse.json({
        agreement: {
          status: 'active',
          customerId: customer.id,
    ...
    if (pastDueSubscription) {
      ...
      // Get the latest invoice for retry payment
    ...
    // Cancelled subscription (show this if no other active ones)
    ...
    // Customer exists but no pending agreement
    return NextResponse.json({ agreement: null });
