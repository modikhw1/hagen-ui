...
/**
 * Load customer-specific concepts with customizations
 */
export async function loadCustomerConcepts(customerProfileId: string): Promise<TranslatedConcept[]> {
  try {
    const supabase = getSupabaseClient();

    // Get customer concepts with base concept data
    const { data: customerConcepts, error } = await supabase
    ...
    if (error) {
      console.error('[conceptLoaderDB] Error loading customer concepts:', error);
      return [];
    }

    // Transform to TranslatedConcept with customer customizations
    return customerConcepts.map(cc => {
      const conceptData = (cc as any).concepts;
      const clip = conceptData.backend_data as BackendClip;
      const override = conceptData.overrides as ClipOverride;

      // Translate base concept
      const translated = translateClipToConcept(clip, override);

      // Apply customer-specific customizations
      return {
        ...translated,
        headline_sv: cc.custom_headline || translated.headline_sv,
        description_sv: cc.custom_description || (translated as any).description_sv,
        whyItWorks_sv: cc.custom_why_it_works || translated.whyItWorks_sv,
        matchPercentage: cc.match_percentage || translated.matchPercentage,
        // Add custom fields
        customInstructions: cc.custom_instructions,
        customTargetAudience: cc.custom_target_audience,
        customScript: cc.custom_script,
        customProductionNotes: cc.custom_production_notes,
        customerNotes: cc.notes,
      };
    });
  } catch (error) {
    console.error('[conceptLoaderDB] Fatal error:', error);
