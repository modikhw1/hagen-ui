...
function getInvoiceSubscriptionId(invoice: Stripe.Invoice): string | null {
  ...
  return typeof directSubscription === 'string' ? directSubscription : directSubscription?.id ?? null;
}
...
function isSubscriptionDefinitelyEnded(subscription: Stripe.Subscription, mirroredStatus: string) {
  ...
  return mirroredStatus === 'canceled' || Boolean(subscription.ended_at) || Boolean(subscription.canceled_at);
}
...
export async function upsertInvoiceMirror(params: {
  ...
  const { supabaseAdmin, invoice, environment, profileIdMap } = params;
  const stripeCustomerId = typeof invoice.customer === 'string' ? invoice.customer : invoice.customer?.id ?? null;
  const stripeSubscriptionId = getInvoiceSubscriptionId(invoice);

  let customerProfileId: string | null = null;
  if (stripeCustomerId) {
    if (profileIdMap?.has(stripeCustomerId)) {
      customerProfileId = profileIdMap.get(stripeCustomerId) ?? null;
    } else {
  ...
  const subtotalOre =
    typeof invoice.subtotal === 'number' && Number.isFinite(invoice.subtotal) ? invoice.subtotal : invoice.amount_due;
  const totalOre =
    typeof invoice.total === 'number' && Number.isFinite(invoice.total) ? invoice.total : invoice.amount_due;
  const taxOre = Math.max(0, totalOre - subtotalOre);

  // The risky `upsertWithMissingColumnFallback` helper has been removed.
  // If an upsert fails due to a missing column, it indicates a schema mismatch
  // between the application code and the database. This is a critical error
  // that should fail loudly and be fixed via a migration, not swallowed silently.
  ...
  if (error) {
    throw new Error(`Failed to sync invoice mirror (ID: ${invoice.id}): ${error.message}`);
  }
...
export async function upsertSubscriptionMirror(params: {
  ...
  const { supabaseAdmin, subscription, environment, profileIdMap } = params;
  ...
  if (stripeCustomerId) {
    if (profileIdMap?.has(stripeCustomerId)) {
      customerProfileId = profileIdMap.get(stripeCustomerId) ?? null;
      // Note: We don't fetch operationalProfile here for performance. Bulk sync relies on this.
    } else {
      const { data } = await supabaseAdmin
        .from('customer_profiles')
  ...
  const mirroredStatus = subscription.pause_collection ? 'paused' : subscription.status;
  ...
  const payload = {
    ...
    scheduled_price_change: operationalProfile ? buildScheduledPriceChange(operationalProfile) : null,
    environment,
  ...
  if (error) {
    throw new Error(`Failed to sync subscription mirror (ID: ${subscription.id}): ${error.message}`);
  }

  if (customerProfileId) {
    const currentCustomerProfileRes = await supabaseAdmin
      .from('customer_profiles')
    ...
    const currentCustomerProfile = currentCustomerProfileRes.data;
    if (currentCustomerProfile && currentCustomerProfile.status !== 'archived' && isSubscriptionDefinitelyEnded(subscription, mirroredStatus)) {
      ...
      await recordAuditLog(supabaseAdmin, {
        ...
        beforeState: currentCustomerProfile as Record<string, unknown>,
        afterState: (archivedProfile ?? { id: customerProfileId, status: 'archived', next_invoice_date: null }) as Record<string, unknown>,
        metadata: {
          stripe_subscription_id: subscription.id,
