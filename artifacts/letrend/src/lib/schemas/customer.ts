...
export const customerInviteSchema = z
  .object({
...
export const sendInviteActionSchema = customerInviteSchema
  .omit({
    discount_type: true,
    discount_value: true,
...
const createCustomerBaseSchema = customerInviteSchema
  .extend({
...
export const createCustomerServerSchema = createCustomerBaseSchema
  .extend({
    send_invite_now: z.boolean().optional(),
    send_invite: z.boolean().optional(),
  })
  .strict()
  .transform(({ send_invite, send_invite_now, ...rest }) => {
    if (send_invite !== undefined) {
      console.warn(
        '[deprecated] createCustomer payload field "send_invite" is deprecated; use "send_invite_now".',
      );
    }

    return createCustomerSchema.parse({
      ...rest,
      send_invite_now: Boolean(send_invite_now || send_invite),
    });
  });
...
export type CreateCustomerServerPayload = z.infer<typeof createCustomerServerSchema>;
